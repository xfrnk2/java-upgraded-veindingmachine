...
- [x] 자판기 보유 금액을 입력 받고 최소 갯수의 동전을 생성하는 기능
    ```
    [예외]
    - 입력이 숫자가 아니면 예외처리
    - 입력이 숫자이지만 0보다 작으면 예외처리
    - 입력 숫자가 최소 동전 단위로 나누어 떨어지지 않으면 예외처리(낙전이 발생)
    ```

- [x] 자판기가 보유한 동전을 출력하는 기능
- [x] 각 상품명, 가격, 수량이 적힌 상품 목록을 입력받아 등록하는 기능
    ```
    [예외]
    상품 목록
    - 입력이 빈 값이면 예외처리
    상품
    - 상품 상세의 가장 앞과 뒤가 대괄호로 시작해서 끝나지 않으면 예외처리
    - 상품 상세를 콤마(,)로 나누었을 때 상품명, 가격, 수량의 총 세 단위가 아니면 예외처리 
    - 가격 또는 수량이 숫자가 아닌 경우 예외처리
    - 가격이 100원 이상이 아니거나, 10원으로 나누어 떨어지지 않으면 예외처리
    - 수량이 0보다 작으면 예외처리
    ```
- [x] 투입 금액을 입력 받는 기능
    ```
    ...
    - 입력이 0보다 작으면 예외처리
    ```
- [x] 구매할 상품명을 입력받아 상품을 구매하는 기능
    ```
    [예외]
    - 구매할 상품명이 존재하지 않는 상품명이면 예외처리
    - 구매할 상품이 수량이 0이면 예외처리
    - 잔여 투입 금액보다 구매할 상품의 가격이 더 비싸면 예외처리(구매 불가능)
    ```
- [x] 반환할 잔돈을 구하는 기능
- [ ] 반환할 잔돈을 출력하는 기능
